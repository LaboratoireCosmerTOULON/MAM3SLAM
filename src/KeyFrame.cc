...
KeyFrame::KeyFrame(Agent* pAgent, long unsigned int nAgentLevelKFid):
        ...
        NLeft(0),NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), mpAgent(pAgent), mnAgentLevelId(nAgentLevelKFid)
{

}

KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, Agent* pAgent, long unsigned int nAgentLevelKFid):
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), mpAgent(pAgent), mnAgentLevelId(nAgentLevelKFid)
{
    mnId=nNextId++;
