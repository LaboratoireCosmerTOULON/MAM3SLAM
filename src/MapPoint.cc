...
MapPoint::MapPoint():
    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mnFirstKFAgentId(-1), mnFirstKFagentLevelId(-1)
{
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mnOriginMapId(pMap->GetId()), mnFirstKFAgentId(pRefKF->getAgent()->mnId), mnFirstKFagentLevelId(-1)
{
    SetWorldPos(Pos);
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId()), mnFirstKFAgentId(-1), mnFirstKFagentLevelId(-1)
{
    SetWorldPos(Pos);
