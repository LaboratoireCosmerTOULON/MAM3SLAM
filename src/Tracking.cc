    ...
    else // FIXME : uncomment and update when current map / agent linkage ok
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if((!mbVelocity && !pCurrentMap->isImuInitialized()) || mnFramesSinceLastReloc<2) // FIXME : bad condition. Need to set a counter instead of relying on ids when working on reloc
                {
...
void Tracking::CreateNewKeyFrame() // don't forget to increase mnFramesSinceLastReloc !!
{
    if(mpLocalMapper->IsInitializing() && !mpAtlas->isImuInitialized()) {
        // std::cout << "LM inititializing : returning" << std::endl; // DEBUG
        return;
    }
