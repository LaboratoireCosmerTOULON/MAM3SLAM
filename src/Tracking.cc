...
{
    // Create KeyFrames
    KeyFrame* pKFini = new KeyFrame(mInitialFrame,mpAtlas->GetAgentCurrentMap(mpAgent),mpKeyFrameDB, mpAgent, mnNextAgentLevelKFid);
    mnNextAgentLevelKFid++;
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame,mpAtlas->GetAgentCurrentMap(mpAgent),mpKeyFrameDB, mpAgent, mnNextAgentLevelKFid);
    mnNextAgentLevelKFid++;
    ...
    std::cout << "Connections updated" << std::endl; // DEBUG
    ...
    pKFcur->SetPose(Tc2w);
    // std::cout << "Tc2w ok" << std::endl; // DEBUG
...
void Tracking::UpdateLastFrame() // Concretement, ne fait rien si mono sauf set la pose de mLastFrame via mlRelativeFramePoses
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            ...
            MapPoint* pNewMP = new MapPoint(x3D,mpAtlas->GetAgentCurrentMap(mpAgent),&mLastFrame,i);
...
bool Tracking::NeedNewKeyFrame() // seems ok
{
    ...
    const bool ctest = mpAtlas->KeyFramesInMap(mpAgent)>5;
    const bool c1d = (mnKFinsertionsRefusedSinceLast>5); // && ctest;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2 = (((mnMatchesInliers<nRefMatches*thRefRatio || bNeedToInsertClose)) && mnMatchesInliers>15);
    const bool c5 = (mnMatchesInliers<100 && mnMatchesInliers>15); // && ctest; // PB : AJOUTES MAIS PAS PROCESS CAR BCP
...
void Tracking::CreateNewKeyFrame() // don't forget to increase mnFramesSinceLastReloc !!
{
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpAtlas->GetAgentCurrentMap(mpAgent),mpKeyFrameDB, mpAgent, mnNextAgentLevelKFid);
    mnNextAgentLevelKFid++;
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(bTcw)
            {
                ...
                {
                    bMatch = true;
                    mpAgent->SetCurrentMap(vpCandidateKFs[i]->GetMap());
                    break;
                }
            }
        }
